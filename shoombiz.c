...
int fileExists(const char *filePath) // do we have this file or not
{
    return access(filePath, F_OK) == 0;
}

void removeSpaces(char *str) // this can remove spaces between words of sentence
{
    int i, j;
...
int hasDot(char str[])
{ // for having list of commit file files we need to  just have files not directories(we have dot in name of files)
    int i = 0;
    while (str[i] != '\0')
    {
        if (str[i] == '.')
...
void copyFolder(char source[], char destination[]) // this give 2 addresses and copy one to anotherone
{
    char command[100];
...
int compareTimes(char *time1, char *time2) // it calculates the total second that any time has and compare them it is for log since
{
    int hour1, min1, sec1;
    int hour2, min2, sec2;
...
int compareTimes2(char *time1, char *time2) // it is for comparing time for before
{
    int hour1, min1, sec1;
    int hour2, min2, sec2;
    sscanf(time1, "%d:%d:%d", &hour1, &min1, &sec1);
...
void printSentencesInReverseOrder(FILE *file) // this can print sentences in reverse order
{
...
void ReverseFile(FILE *file) // this make a temp_log file to have reverse of message file
{
...
void reverseForLog(FILE *file, int number) // this just print ad much as we want for log -n
{
    char sentences[MAX_SENTENCES][100];
...
void replaceDotWithNull(char *str) // it deletes members after dot and also dot
{
    while (*str != '\0')
    {
        if (*str == '.')
...
bool directoryExists(char *path) // this recognize if we have a dir or not by using its path
{
    struct stat info;
    if (stat(path, &info) != 0)
    {
...
void search(char user[200], char saved[200]) // it can saparate parts between ""
{
    int start = 0;
    int end = 0;
...
int match(char *pattern, char *text) // it is used for wild cards
{
    // for null characters.
    if (*pattern == '\0' && *text == '\0')
    {
...
int token_del(char *name_of_files[1000], char user[100]) // it is for saparating parts between <>
{
    int count = -1;
    int input = 0;
    char del[100] = "<>";
...
void addBackslashes(char *str) // for understanding C the address we should add / to them
{
...
void reset_for_directory(const char *filename, char *name) // the first input=address of staging folder and the second is the folder
{
    char currentDirectory[FILENAME_MAX];
